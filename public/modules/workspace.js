      ...
      for (const vulnerability of arrayValue(pkg.vulnerabilities)) {
        ...
        for (const id of ids) {
          rows.push(normalizeExposure({
            ...
            fixProvenance: vulnerability.fixProvenance,
            purl: pkg.purl,
            source: "OSV",
...
function normalizeExposure(value) {
  ...
  return {
    ...
    fixedVersions: unique(arrayValue(value.fixedVersions || value.fixedVersion).map((item) => String(item || "").trim()).filter(Boolean)),
    fixProvenance: arrayValue(value.fixProvenance).slice(0, 20),
